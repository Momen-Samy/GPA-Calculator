...
export default function Semesters() {
  ...
  const totalGPA = useMemo(() => {
    ...
    const totalGpa =
      totalPoints / totalHours ? (totalPoints / totalHours).toFixed(2) : "0.00";
  ...
  const addNewForm = (semesNum: number) => {
    const semesClone = { ...semester };
    const formKeys = Object.keys(semesClone[semesNum].forms).map(
      (elem) => +elem
    );
    let Min = 1;
    for (; ; Min++) {
  ...
  const removeSemester = (semesNum: number) => {
    setSemester((semester) => {
      const semesClone = { ...semester };
  ...
  const removeForm = (formNum: number, semesNum: number) => {
    const semesClone = { ...semester };
    ...
    semesClone[semesNum].semes = calcGPA__For__Semes(semesNum, semesClone);
    localStorage.setItem("semester", JSON.stringify(semesClone));
    setSemester(semesClone);
  };
  ...
  const calcGPA__For__Semes = (semesNum: number, semesClone: Semestertype) => {
    let totalHours = 0;
    let totalPoints = 0;
  ...
  return (
    <>
      <div className={styles["GPA-box"]}>
          ...
          <div className="semester-box">
            {Object.keys(semester).map((semesNum) => (
              <div key={semesNum} className={styles["semes"]}>
                ...
                {Object.entries(semester[+semesNum].forms).map(
                  ([formNum, [courseName, grade, creditHr]]) => (
                    <form key={+formNum}>
                      <input
                        type="text"
                ...
                <div className={styles["calcAndadd"]}>
                  <p>
                    {`Semester ${semesNum} GPA: ${
                      semester[+semesNum].semes.semsGPA
                    }`}
                  </p>
                  <button
        ...
        <button className={styles["add-semester"]} onClick={addNewSemester}>
          <i>
            <IoAddCircleSharp />
          </i>
          <p>add semester</p>
